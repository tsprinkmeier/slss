...
public:
  ...
  // GF inverse log
  uint8_t ilog(const uint8_t a) const
    {
      return gfilog[a];
    };

  // fast mult, just use the lookup table
  inline uint8_t mult(const uint8_t a, const uint8_t b) const
    {
      return multLookup[(a << 8) + b];
    };

  // slow multiplication
  uint8_t slowMult(const uint8_t a, const uint8_t b) const
    {
      // (0 * 0) == (a * 0) == (0 * b) == 0
      if (!a || !b)
      {
...
private:
  ...
  static void test(const uint8_t a,
                   const uint8_t b,
                   const uint8_t c,
                   const uint8_t d,
                   const char * msg)
  ...
  void BIT() const
    {
      ...
      for (uint8_t a = 1; a; a++)
      {
        ...
        */
        ...
        for (uint8_t b = 1; b; b++)
        {
          const uint8_t c = mult(a,b);
          const uint8_t d = mult(b,a);
          ...
          {
            // verify that ((a * b)/a) == b
            const uint8_t e = div(c,a);
            test(a, b, e, b, "(a*b)/a != b");
          }
        }
      }
    };
